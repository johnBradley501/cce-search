...
The Python code makes use of the module "MySQLdb" to support access to the MySQL database.
...
The Python files here generate JSON files (well, strictly speaking, JavaScript files -- see below). Some code on `common.py` manages this process.
...
value places the generated files directly in the folder on my machine where they need to be to be to be used by the JavaScript search mechanism. You should
